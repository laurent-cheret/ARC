...
def connect(grid_lists):
    ...
    def process_single_grid(grid):
        ...
        def find_closest_points(points1, points2):
            ...
            for p1 in points1:
                for p2 in points2:
                    distance = abs(int(p1[0]) - int(p2[0])) + abs(
                        int(p1[1]) - int(p2[1])
                    )
        ...
        def draw_path(grid, from_point, to_point, digit):
            ...
            while not torch.all(current == to_point):
                if abs(int(current[0]) - int(to_point[0])) > abs(
                    int(current[1]) - int(to_point[1])
                ):
                    step = torch.tensor(
                        [1 if to_point[0] > current[0] else -1, 0], dtype=torch.long
                    )
                else:
                    step = torch.tensor(
                        [0, 1 if to_point[1] > current[1] else -1], dtype=torch.long
                    )
        ...
        for digit, points in objects.items():
            ...
            for i in range(len(points) - 1):
                from_point, to_point = find_closest_points(
                    points[i].unsqueeze(0), points[i + 1 :]
                )
    ...
    def process_single_grid(grid):
        ...
        def draw_straight_path(from_point, to_point, digit):
            ...
            if from_point[0] == to_point[0]:  # Same row
                ...
                start, end = min(from_point[0], to_point[0]), max(
...
def connect_to_source(grid_lists):
    ...
    def process_grid_pair(grid0, grid1):
        ...
        source_coords = np.argwhere(
            source_labeled == 1
        )  # Assuming the first object is the source
                ...
                for source_coord in source_coords:
                    if coord[0] == source_coord[0] or coord[1] == source_coord[1]:
                        ...
                        if coord[0] == source_coord[0]:  # Same row
                            start, end = min(coord[1], source_coord[1]), max(
                                coord[1], source_coord[1]
                            )
                            connection_grid[coord[0], start : end + 1] = color
                        else:  # Same column
                            start, end = min(coord[0], source_coord[0]), max(
                                coord[0], source_coord[0]
                            )
...
def move_obj_to_source(grid_lists):
    ...
    return [
        (
            [process_grid_pair(grid_list[0], grid_list[1])] + grid_list[2:]
            if len(grid_list) > 1
            else grid_list
        )
        for grid_list in grid_lists
    ...
    def _process_torch_grid(grid):
        ...
        for i in range(min_row, -pattern_h, -pattern_h):
            for j in range(min_col, -pattern_w, -pattern_w):
                new_grid[
                    max(0, i) : min(h, i + pattern_h), max(0, j) : min(w, j + pattern_w)
                ] = pattern[
                    max(0, -i) : pattern_h - max(0, i + pattern_h - h),
        ...
        for i in range(min_row, -pattern_h, -pattern_h):
            for j in range(min_col, w, pattern_w):
                new_grid[
                    max(0, i) : min(h, i + pattern_h), j : min(w, j + pattern_w)
                ] = pattern[
                    max(0, -i) : pattern_h - max(0, i + pattern_h - h),
                    : min(w - j, pattern_w),
                ]
        ...
        for i in range(min_row, h, pattern_h):
            for j in range(min_col, -pattern_w, -pattern_w):
                new_grid[
                    i : min(h, i + pattern_h), max(0, j) : min(w, j + pattern_w)
                ] = pattern[
                    : min(h - i, pattern_h),
                    max(0, -j) : pattern_w - max(0, j + pattern_w - w),
                ]
    ...
    def process_single_grid(grid):
        ...
        for i in range(1, num_features + 1):
            ...
            if height % 2 == 1 and width % 2 == 1:
                ...
                center_grid[center_y : center_y + 2, center_x : center_x + 2] = np_grid[
                    center_y : center_y + 2, center_x : center_x + 2
                ]
            elif height % 2 == 0:
                # Even x Odd: 2x1 central region
                center_y, center_x = int(center[0] - 0.5), int(center[1])
                center_grid[center_y : center_y + 2, center_x] = np_grid[
                    center_y : center_y + 2, center_x
                ]
            else:
                # Odd x Even: 1x2 central region
                center_y, center_x = int(center[0]), int(center[1] - 0.5)
                center_grid[center_y, center_x : center_x + 2] = np_grid[
                    center_y, center_x : center_x + 2
                ]
        ...
        for i in range(height):
            for j in range(width):
                ...
                if zero_count > non_zero_count:
                    ...
                    non_zero_surrounding = [
                        cell for cell in surrounding_cells if cell != 0
                    ]
