...
def rotate_90(grid_lists):
    ...
    """
    return [[torch.rot90(grid, 1, [0, 1]) for grid in grid_list] for grid_list in grid_lists]

def rotate_180(grid_lists):
    """
    Rotates each grid in the grid lists by 180 degrees.
    """
    return [[torch.rot90(grid, 2, [0, 1]) for grid in grid_list] for grid_list in grid_lists]

def rotate_270(grid_lists):
    """
    Rotates each grid in the grid lists by 270 degrees clockwise (or 90 degrees counter-clockwise).
    """
    return [[torch.rot90(grid, 3, [0, 1]) for grid in grid_list] for grid_list in grid_lists]
...
def diagonal(grid_lists):
    ...
    def get_upper_diagonal(grid):
        ...
        return result
    return [[get_upper_diagonal(grid) for grid in grid_list] for grid_list in grid_lists]
...
def pad_up(grid_lists):
    ...
    def pad_grid_up(grid):
        if grid.shape[0] < MAX_SIZE:
            pad_row = torch.zeros((1, grid.shape[1]), dtype=grid.dtype, device=grid.device)
...
def pad_left(grid_lists):
    ...
    def pad_grid_left(grid):
        if grid.shape[1] < MAX_SIZE:
            pad_col = torch.zeros((grid.shape[0], 1), dtype=grid.dtype, device=grid.device)
...
def _logical_operation(grid_lists, operation):
    ...
    def process_grid_list(grid_list):
        ...
        if operation == 'OR':
            ...
            result = torch.where((grid0 != 0) & (grid1 != 0), grid0, torch.zeros_like(grid0))
        elif operation == 'XOR':
...
def _resize_grid(grid, scale_factor):
    ...
    if isinstance(grid, torch.Tensor):
        ...
        return [[cell for cell in row for _ in range(scale_factor)] for row in grid for _ in range(scale_factor)]
...
def resize_by_count(grid_lists):
    ...
    def process_grid(grid):
        ...
        return _resize_grid(grid, scale_factor) if _check_size(grid, scale_factor) else grid
