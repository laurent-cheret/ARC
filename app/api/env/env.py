...
class TransformerEncoder(nn.Module):
    def __init__(
        self, input_dim, d_model, nhead, num_layers, dim_feedforward, dropout=0.0
    ):
...
class GridTransformationEnv(gym.Env):
    def __init__(
        ...
        self.memory_state = np.zeros(
            (memory_capacity, self.encoding_dim), dtype=np.float32
        )
        # Load all primitives
        ...
        self.observation_space = spaces.Dict(
            {
                "current_objective": spaces.Box(
                    low=-np.inf,
                    high=np.inf,
                    shape=(self.encoding_dim,),
                    dtype=np.float32,
                ),
                "current_step": spaces.Discrete(
                    max_steps
                ),  # Assuming a maximum of 100 steps per episode
                "num_train_examples": spaces.Discrete(
                    11
                ),  # Assuming a maximum of 10 training examples
                "quantity_error": spaces.Box(
                    low=0, high=1, shape=(1,), dtype=np.float32
                ),
                "original_colors": spaces.Box(
                    low=0, high=1, shape=(10,), dtype=np.int32
                ),
                "current_colors": spaces.Box(
                    low=0, high=1, shape=(10,), dtype=np.int32
                ),
                "memory_state": spaces.Box(
                    low=-np.inf,
                    high=np.inf,
                    shape=(memory_capacity, self.encoding_dim),
                    dtype=np.float32,
                ),
                "avg_width_difference": spaces.Box(
                    low=-np.inf, high=np.inf, shape=(1,), dtype=np.float32
                ),
                "avg_height_difference": spaces.Box(
                    low=-np.inf, high=np.inf, shape=(1,), dtype=np.float32
                ),
    ...
    def encode_grid(self, grid):
        ...
        with torch.no_grad():
            grid_tensor = grid_tensor.view(1, -1).to(
                self.device
            )  # Reshape to (1, 9000)
    ...
    def load_all_primitives(self):
        ...
        for filename in os.listdir(dsl_dir):
            if (
                ...
                module_name = (
                    f"dsl.{filename[:-3]}"  # Strip '.py' extension to get module name
                )
    ...
    def _get_observation(self):
        ...
        current_colors = self._compute_color_usage(
            [grid for sublist in self.current_grids for grid in sublist]
        )
        ...
        obs = {
            ...
            "demonstration_action": (
                self.current_demonstration[self.demonstration_step]
                if self.using_demonstration
                and self.demonstration_step < len(self.current_demonstration)
                else len(self.primitives) - 1
            ),
        }
    ...
    def _compute_general_task_intuition(self):
        intuitions = [
            out - inp for inp, out in zip(self.initial_encodings, self.output_encodings)
        ]
    ...
    def _compute_current_intuition(self):
        ...
        for i, initial_encoding in enumerate(self.initial_encodings):
            current_encodings = [
                self.encode_grid(grid) for grid in self.current_grids[i]
            ]
    ...
    def _compute_reward(self, obs):
        ...
        for i in range(num_train_examples):
            ...
            current_grid = self.current_grids[i][
                0
            ]  # Take only the first grid from each list
        ...
        if correct_grids > 0:
            return correct_grids / (
                num_train_examples * self.current_step
            )  # We impose a discount on the reward, to focus on
        ...
        for task_id, action_names in demo_data.items():
            ...
            for action_name in action_names:
                if action_name in self.primitives_names:
                    ...
                    print(
                        f"Warning: Action '{action_name}' not found in primitives for task {task_id}"
                    )
            if valid_actions:
